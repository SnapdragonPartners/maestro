...
- **PR CI is read-only.** CI pulls the committed digest and runs `cache-verify`; it never pushes and never rewrites the digest. A re-pin that leaves the image without coverage of the current pins is caught loudly by the offline check; a re-pin it still covers needs no republish (amended 2026-07-22, PROPOSED). Coverage is the trigger, not the pin string: a re-pin the image still covers needs no republish (amended 2026-07-22, proposed — see design_cost_latency.md). The check proves coverage, not input equality.
