...
func TestLoadOnlyRefusesAMissingKey(t *testing.T) {
	...
	if _, statErr := os.Stat(filepath.Join(root, KeyFileName)); !os.IsNotExist(statErr) {
...
// TestKeyFileProviderNamesItsBackend: a diagnostic that asks which source
// answered must get the key file, not a guess.
func TestKeyFileProviderNamesItsBackend(t *testing.T) {
	if got := KeyFile(t.TempDir(), LoadOnly).Backend(); got != secret.BackendKeyFile {
		t.Fatalf("key-file provider reports backend %q, want %q", got, secret.BackendKeyFile)
	}
}
...
func TestUnimplementedBackendsRefuseAtConstruction(t *testing.T) {
	for _, backend := range []secret.Backend{secret.BackendKeychain, secret.BackendPassphrase} {
		root := t.TempDir()

		provider, err := ProviderFor(backend, root, MayCreate)
		if !errors.Is(err, secret.ErrBackendNotImplemented) {
...
func TestUnknownBackendIsAConstructionError(t *testing.T) {
	if _, err := ProviderFor(secret.Backend("vault-server"), t.TempDir(), LoadOnly); err == nil {
