...
| [0026](0026-multi-architecture-artifacts.md) | Multi-Architecture Distributable Artifacts | Proposed | Every artifact built on one arch and executed on another — embedded binaries and published images — must be multi-arch (amd64 + arm64), pinned by manifest digest, and verified per-arch; single-arch cross-arch artifacts are a defect. Recurred in the MCP proxy and the benchmark cache image. |
