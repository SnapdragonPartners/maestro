...
| [0028](0028-artifact-envelopes-and-payload-schemas.md) | Artifact Envelopes And Payload Schemas | Draft | The encoding layer under ADR 0021: a fixed relational envelope plus a typed JSON payload, digested over RFC 8785 canonical JSON; a code-resident payload type registry validated at the persistence seam on write; additive-within-version evolution where the reader is the only compatibility layer, because accepted artifacts are never rewritten; amendments as RFC 7386 merge patches applied in sequence to materialize the effective view on read; review records bound to the exact payload digest reviewed. |
