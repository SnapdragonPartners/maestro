...
- [Maestro v2 ADR Backlog](notes_adr-backlog.md) — Reconciled, stable-numbered ADR backlog (Phase 0 item 12): candidates resolved in Phase 0 and in later phases with their Accepted ADRs, and open candidates each labelled with the phase it blocks — slot numbers are cited by other documents and never change, so a heading rather than a position in the list is what says when a candidate is due.
- [Conformance Run Log](notes_conformance-log.md) — The committed digest of every phase-end golden-story conformance run: date, target identity, per-story verdict, and cost/token totals. Raw records remain in the runner's self-contained store and, from Phase 2 onward, are also imported into the main Postgres data plane as queryable first-class artifacts.
...
Per-phase working artifacts (scope/plan, spike reports, inventories, manifests) live in `phase_x/` directories matching the branch namespace, each with its own README index — see [phase_0/](phase_0/README.md), [phase_1/](phase_1/README.md), [phase_2/](phase_2/README.md), and [phase_3/](phase_3/README.md). Cross-phase documents stay at this root. Accepted decisions land in `docs/adr/`.
