...
- [Maestro v2 ADR Backlog](notes_adr-backlog.md) — Reconciled, dependency-ordered ADR backlog (Phase 0 item 12): candidates resolved in Phase 0 and in later phases with their Accepted ADRs, and open candidates labelled with the phase they block — numbers are stable and cited elsewhere, so each heading rather than its ordinal position is authoritative about phase.
