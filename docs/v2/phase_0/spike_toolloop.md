...
- **Three genuine gaps block convergence today**, not one:
  1. *Early termination*: the toolkit loop terminates only on a no-tool final answer, max iterations, or error — no affordance a terminal tool could trigger. A context-cancellation hack would conflate terminal exit with user cancellation.
  2. *Pre-request extension*: the toolkit copies its transcript (immutable by design) and its hooks fire post-response only, while Maestro flushes buffered human input, injects pre-iteration guidance, and records activity heartbeats *before* each provider call.
  3. *Infallible hooks*: the observation hooks cannot return errors, so nothing durable can ride them. (Durable audit has an adapter-side answer regardless: persistence wraps the adapted `Execute`, which is synchronous and error-capable.)
  The last two are plausibly general-purpose upstream reviews, not Maestro special-pleading — many consumers will eventually want a controlled pre-request point and fallible hooks.
- **Escalation maps without upstream changes**: soft limits ride the toolkit's `OnIteration` counter (the warning callback is Maestro-side and fires between iterations); the hard limit maps onto `MaxIterations`, with the harness layer converting the `max_iterations` outcome into Maestro's escalation semantics.
...
2. **Sort every Maestro-loop feature by upstream candidacy**, not by current toolkit non-goals:
   - **Required upstream requests**: an early-termination affordance (a `ToolResult` terminal flag or stop sentinel), and a controlled pre-request extension point (message injection/flush plus pre-call observation). Alongside these, request a review of the hooks-cannot-return-errors design — fallible hooks are a reasonable multi-consumer need, not Maestro special-pleading.
   ...
   - **Composable from existing hooks**: escalation soft limits ride `OnIteration`; the hard limit maps onto `MaxIterations` with the harness layer translating the outcome.
