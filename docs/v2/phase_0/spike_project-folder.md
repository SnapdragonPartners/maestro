...
4. **Retire the "project directory" concept entirely** (per DR, on review). It existed for a v1 assumption that no longer holds — operating on an already-materialized user checkout. v2 never does that: Maestro works only from its own mirrors and clones (ADR 0023), and a user's existing checkout is irrelevant to it. The taxonomy broke the model independently: a Product spans repositories, so no single folder can host "the project." What remains locally is split by function into OS-standard locations:
   ...
   - Caveat (per DR): "cache" here means *reconstructible to the last durable point*, not backed-up. In-flight, unpushed workspace changes are the one thing in it that exists nowhere else — losing the cache mid-Story means replaying from the last pushed branch or accepted artifact, not data loss from the data plane. The branch workflow bounds the exposure (pushes at every review point), and OSes are gentle with these directories in practice — but macOS *can* purge caches under storage pressure, so if lived experience shows purges hurting, the fallback is a non-purgeable default location with the same disposability semantics. Flagged for Phase 3's workspace design, not decided here.
   - **Maestro data** — an OS user-data location holding the bind-mounted Postgres and object-store volumes (ADR 0022 as amended: data-plane volumes are never anonymous or Docker-Desktop-internal, so restarts, container recreation, and Desktop resets cannot destroy the data plane). The only precious local bytes; backing up Maestro means backing up this one directory.
