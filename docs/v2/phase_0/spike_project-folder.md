...
- **The non-recreatable core is six files**: `maestro.db` (SQLite — already a database), `config.json`, `secrets.json.enc` (encrypted), `forge_state.json`, `.password-verifier.json`, and the user-authored `.env`. Plus `.maestro/knowledge.dot`, which is repo-committed and whose design dies with v1 anyway (the cms spike moves v2 knowledge to the data plane).
...
**Yes: the v2 project folder is disposable.** The parking-lot hypothesis is confirmed by inventory — what remains local is the bootstrap pointer, the root-of-trust key file, active workspaces until their work is pushed, and recreatable caches; nothing durable.
...
1. **To the data plane** (resolving ADR 0022's deliberate deferral: the schema *should* hold config and credentials, not merely be capable of it):
   ...
   - Knowledge → the data plane per the cms spike; `.maestro/knowledge.dot` retires with v1's design.
...
4. **Retire the "project directory" concept entirely** (per DR, on review). It existed for a v1 assumption that no longer holds — operating on an already-materialized user checkout. v2 never does that: Maestro works only from its own mirrors and clones (ADR 0023), and a user's existing checkout is irrelevant to it. The taxonomy broke the model independently: a Product spans repositories, so no single folder can host "the project." What remains locally is split by function into OS-standard locations:
   - **Maestro config** — `os.UserConfigDir()/maestro` (macOS `~/Library/Application Support`, XDG config on Linux): the data-plane bootstrap pointer and the root-of-trust key file, nothing else.
   ...
   - A `MAESTRO_HOME` override collapses all four under one directory (the classic `~/.maestro` layout) for those who want it — but as `config/`, `cache/`, `state/`, and `data/` *subdirectories*, never flattened: the semantics travel with the split, so cold backup still copies only `data/` and still excludes the key.
5. **The naming tangle resolves as a side effect**: with the local state hub retired and local directories named by function (config, cache, state, data), the committed repo `.maestro/` directory becomes the only thing *Maestro itself* names `.maestro` — repo-scoped project artifacts, exactly like `.github/`. (A user setting `MAESTRO_HOME=~/.maestro` reintroduces the name by their own choice; Maestro's vocabulary and defaults carry exactly one `.maestro`.) No rename needed; the collision dies because one of the colliding parties does. Retired from the vocabulary: "project directory."
