...
| D6 | PR #346 review (Copilot): `WithRemedy` carried the error it was handed, whose chain already contained the readiness error — so rendering walked it and the operator read the whole refusal **twice, with the superseded remedy last**. It now carries the producer's diagnostic (`r.Err`): one rendering, sentinel still reachable by `errors.Is`. The cost is stated as a rule — a composer with context to add puts it in the Detail, not in a wrapper. |
| D4 | PR #346 review (CodeQL): `Embedded`'s version parse was unbounded at 64 bits and fed a `uint` conversion, which truncates on a 32-bit platform — a binary would believe its own schema older than it is. The parse is bounded at 32 bits in a `parseMigrationVersion` helper the test can drive, so the mutation is caught on 64-bit hosts too, where the truncation itself is unobservable. |
