...
- **Import graph.** `go list` over `./...`, capturing `Imports`, `TestImports`
  ...
  have a consumer.
- **Tag completeness.** The graph is computed under **every build tag in the
  repository**, not the default set. See the warning below — this is not a
  detail.
- **Reachability is a proxy.** "Zero importers" supports "nothing imports it",
  not "it is dead". Every retirement below is cross-checked textually before it
  is written down, and the two checks are reported separately.
...
### Warning: build tags change the answer

An import-graph analysis run under the default tag set reports **four packages
as dead that are not**. The repository carries four build tags, and three of
them guard very few files, which is exactly why an analysis misses them:
...
One file, `injection.go`. Zero production importers, zero test importers, under
all four build tags; and zero textual references anywhere outside the package
itself. Phase 0 classified the middleware group **rework** at package grain,
which is right for `metrics` and `validation` and wrong for this member.
...
Evidence column cites what was measured. Rows marked *thin* have a disposition
carried from the plan's hypothesis with package-level evidence only; they need
surface-level evidence before this document leaves draft.
...
| `TerminalTool` | **refactor** | Carries #316's requirement: a terminal tool must be forceable. *thin* |
...
| `MockAgent`, `RunnableMockAgent` exported from the production package | **retire** | Test doubles in the production surface — same class as finding 2. *thin* |
...
| Surface | Disposition | Evidence |
| --- | --- | --- |
| Subprocess integration — `Runner`, `RunOptions`, `Result`, `StreamParser`, stream event types | **retain** | 3,558 lines, 6 files; single importer (`pkg/coder`). Phase 0 "port" confirmed: the external Claude Code subprocess integration keeps working as-is. |
| `Installer`, `embedded` | **retain** | Cross-architecture packaging per ADR 0026. |
| `mcpserver` `lastEffect` and signal-correction path (`SignalDetector`, `SignalToolInput`, `Signal`) | **replace** | `pkg/coder/claude/mcpserver/server.go`. Item 5 removes it; **item 8 builds the real adapter over the contract** — explicitly not a side effect of item 5. |
| MCP JSON-RPC surface (`Server`, `JSONRPCRequest`/`Response`/`Error`) | **refactor** | 935 lines. Tool exposure re-plumbs to v2 tool records (ADR 0022). *thin* |
...
1. Rows marked *thin* carry the plan's hypothesis with package-level evidence
   only. They need surface-level evidence before this document leaves draft.
