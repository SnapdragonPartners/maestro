...
- [Design: Runner Engine And CLI (Item 3)](design_engine.md) — Design sketch for the runner-core work item: attempt lifecycle, verdict composition, budget enforcement split (wall-clock hard, cost post-hoc), isolation and cleanup verification, N-repeat and suite orchestration, the CLI surface, and the one contract addition (Observation.SolutionBranch).
