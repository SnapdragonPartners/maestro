...
- [D9 Sampling And Budget Policy (Item 6)](d9_budget_policy.md) — Real per-story costs from instrumented runs, N fixed at 3/1, and the per-story and per-suite caps fixed as runner-enforced values; caps are a runaway safeguard, not a target. Operational companion: the Run Protocol in [benchmark/README.md](../../../benchmark/README.md).
