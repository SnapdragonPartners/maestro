...
| 5.1 | `cost-latency` | Cut wall-clock and dollar cost before the token-heavy items exercise the harness (benchmark-cost risk, brought forward). Two instrument-economics changes, neither v1 maintenance nor a run-record-contract change: (a) **dependency-cache pre-warming** — pre-baked per-fixture container images (and the bootstrapper baking `go mod download`-style layers into generated dev-container Dockerfiles), so runs stop paying the cold-cache tax ([#268](https://github.com/SnapdragonPartners/maestro/issues/268): discovery-011 lost ~295s, ~40% of wall clock, to a first-run `go mod download`); (b) a **`paired-local` MPH configuration** driving locally-hosted models via Ollama/vLLM (qwen3-coder for the architect role, mistral-small for the others), making basic end-to-end exercise of the harness ~free ([#266](https://github.com/SnapdragonPartners/maestro/issues/266)). Local-model cost reports as `not_applicable` (or a local cost model) while tokens/calls stay `value`. Added 2026-07-17, DR-directed; may warrant its own mini-plan like item 1. | S |
