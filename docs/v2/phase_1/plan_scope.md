...
- [ ] The D9 sampling and budget policy is written down with numbers fixed from instrumented runs, and enforced by the runner (declared budgets, overrun-as-failure). — drafted in [d9_budget_policy.md](d9_budget_policy.md) (item 6, 2026-07-21) and enforced in the story/config TOMLs as provisional safeguards; **stays unchecked until that record is approved and flips from draft to live** (ADR 0017). Caveats recorded there: attempts span three target identities, no attempt ran against the final one, and `cleanup-provider-options` is parked (over-decomposition) rather than calibrated.
