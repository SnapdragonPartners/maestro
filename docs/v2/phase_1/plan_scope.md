...
- [x] The D9 sampling and budget policy is written down with numbers fixed from instrumented runs, and enforced by the runner (declared budgets, overrun-as-failure). — [d9_budget_policy.md](d9_budget_policy.md) (item 6, 2026-07-21); N fixed at 3/1, per-story and per-suite caps enforced. Caveat recorded there: samples are n=1–2, and `cleanup-provider-options` is parked (over-decomposition) rather than calibrated.
