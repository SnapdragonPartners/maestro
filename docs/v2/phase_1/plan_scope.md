...
- [ ] The **v1-derived baseline on `golden-minimal`** is recorded with its full target descriptor (commit hash, binary identity, MPH identity) — a real run on the settled identity, not a citation of the item-6 campaign. **Unsatisfied, and it must be either run or explicitly cancelled; it cannot be declared discharged.** Taking it is cheap (`golden-minimal` is the two cheapest rungs) and it is now takeable for the first time, since the identity settled only after P-11 and the cap fixes. It is also the *last* moment it can be taken: v1's factory path is deleted during the rewrite, so this obligation cannot be deferred to Phase 1B — it expires rather than moves.
