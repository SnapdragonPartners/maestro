...
2. **Schema and versions.** MPH bundle `schema_version` bumps **1 → 2**: adds `local`, and `DeclaredBudget` gains `max_tokens_per_run` and `max_tokens_per_suite` (int64). Existing USD fields are unchanged. Validation is by dimension:
   ...
   - **Local** (`local=true`): require `expected_tokens_per_run>0` (the reservation basis), `max_tokens_per_run>0`, `max_tokens_per_suite>0`; the USD caps **must be absent/zero** — a positive USD cap on a local bundle is a validation error (no ambiguous dual budget).
...
4. **Conservative suite reservation/settlement.** On admission, reserve the effective per-run token max against `max_tokens_per_suite`; admit only if the remaining suite token budget covers the reservation — the token analogue of item 3's "reserve effective per-run max, settle to observed." On completion, settle the reservation down to the run's **observed** tokens (P-1 usage surface).

5. **Unknown-metric behavior.** If observed `tokens_total` is **`unavailable`**, the reservation is **retained** at the effective per-run max, never settled down — conservative, identical to USD-with-unavailable-cost. (Largely moot for local: item 5's P-1 hardening already fails any run whose usage surface never validates, so an unavailable token count means a failed attempt whose reservation releases with it. The retain rule is stated so partial-suite accounting stays conservative regardless.)

6. **Manifest accounting and version.** The suite manifest `manifest_schema_version` bumps **1 → 2**: it gains a `budget_dimension` discriminator (`"usd"` | `"tokens"`) and token accounting fields `cap_tokens`, `charged_tokens`, `observed_tokens`, mirroring the existing `cap_usd`/`charged_usd`/`observed_usd`. For a local config the token fields are authoritative and the USD fields are zero; for a hosted config, vice versa. Budget dimension is **per-config**: a suite mixing hosted and local configs tracks each dimension's reservation independently against that config's own suite cap, and the manifest records both sets (the unused one zero). A reader keys off `budget_dimension`.
...
## Resolutions (Codex Design Rounds 1–2)

The round-1 open questions are settled (round 2's token-budget contract and image-publication lifecycle are specified inline in Parts A and B above):
