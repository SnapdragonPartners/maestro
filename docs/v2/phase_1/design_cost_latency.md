...
The `go.mod`/`go.sum` inputs come from the fixtures at their pinned commits, so the image is a function of the fixtures' module inputs. **Republish trigger is COVERAGE, not commit equality** (amended 2026-07-22, PROPOSED — pending Codex + DR acceptance). Republish when the image no longer covers the fixtures' current pins, as determined by `make cache-verify`. That is exactly what the offline `GOPROXY=off` proof establishes, and deliberately all it claims: coverage does **not** imply the module inputs are unchanged — removing a dependency, or editing only a `go.mod` comment, leaves the existing image a sufficient superset and verification rightly passes. Coverage is the property the image exists to provide, so it is the property the policy is stated in. The original wording — "rebuilt-and-republished whenever any fixture re-pins" — was a conservative proxy that a no-op re-pin (deleting a stray build artifact, say) makes needlessly expensive (owned by the fixture-conventions procedure, [process_fixtures.md](process_fixtures.md)). It is **published to the SnapdragonPartners registry (ghcr.io) and referenced by immutable digest** as the config's `container_image`, so CI and every machine pull byte-identical contents and the digest binds into the harness hash H — same discipline as the base and Gitea pins. One image serves all stories; the cache lives *in the pinned image*, so hermeticity holds — no shared mutable module-cache volume across runs (explicitly rejected as reintroduced cross-run state).
...
- **Maintainer publish (produces the digest).** A make target (`benchmark/`, run by a maintainer or a release workflow holding GHCR push credentials) assembles the fixtures' pinned `go.mod`/`go.sum`, builds the union image, pushes it to `ghcr.io/snapdragonpartners/golden-cache`, and writes the resulting `@sha256:` digest into the config. The image is published **public** — it holds only cached public Go modules, no secrets — so pulls need no auth. This step runs when a re-pin leaves the image without coverage of the current pins — coverage, decided by `cache-verify`, is the trigger, not the pin string (amended 2026-07-22, PROPOSED); a no-op re-pin that the image still covers needs no republish. It is the sole path that rewrites the tracked digest, exactly like the golang-base and Gitea digest pins.
- **PR CI (consumes the digest, read-only).** PR CI **pulls the committed digest and runs the `GOPROXY=off` offline verification** (below). It has no GHCR push credentials, never pushes, and never rewrites the tracked digest. If a re-pin leaves the image without coverage of the current pins, the offline check fails loudly (deps missing from the stale image) — drift cannot merge silently. Coverage is the trigger, not the pin string (amended 2026-07-22, PROPOSED — see the coverage-policy note above): a re-pin the image still covers, such as a no-op that leaves `go.mod`/`go.sum` byte-identical, needs no republish, and the check correctly passes it.
...
- **Union image drift.** An image built from stale `go.mod`/`go.sum` would cache the wrong deps. Mitigation: the `GOPROXY=off` CI proof is the gate — it fails loudly if the image does not cover the current pins, which is the coverage-based trigger above (amended 2026-07-22, proposed). Note its limit: it proves coverage, not input equality, so a shrinking dependency set is *not* detected and does not need to be. The proof fails loudly if a dependency is missing from the cache, so drift cannot pass silently.
