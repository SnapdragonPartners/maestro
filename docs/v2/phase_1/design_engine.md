...
4. **Bind the solution immutably.** A branch name is mutable and proves nothing. The engine requires `SolutionBranch` to be inside the run namespace, resolves it **once** to a commit, verifies the story pin is an ancestor of that commit, and validates at a detached checkout of it — after **cleaning untracked and ignored files**, so validators see exactly the commit and nothing the target left lying around (round 2). For in-place solutions the engine snapshots the working tree as a synthetic commit first, **force-adding ignored files** so nothing outside the recorded commit can influence validation. The resolved commit is recorded as `SolutionCommit`.
5. **Verify.** Engine-executed at the solution commit: the story's validators (commands), then checks (`command`, `files_changed_within` via `git diff --name-only <pin>..<solution-commit>`, `file_contains`); `evidence_shape` and `required_artifacts` must each be covered by the observation's evidence kinds. The wall-clock deadline bounds the whole budgeted phase — run **through verification** — so a hung validator cannot exceed the cap (round 2); deadline expiry anywhere in that span is `budget-overrun`.
...
**Error-path record synthesis.** When `Run` returns an error (including deadline cancellation) or a contract-invalid observation, the engine still produces a complete, valid record: descriptor from step 1; metrics synthesized as `unavailable(reason)` for every capability-declared key and `unsupported` for the rest — except that **streamed usage totals accrued before the abort are retained as measured token/cost values** (failed-attempt costs count, ADR 0025; round 2); verdict `failed` with `budget-overrun` (deadline/usage abort) or `target-error`; evidence whatever the adapter managed to report, else empty.

**Canonical wall-clock.** `wall_clock_seconds` is engine-owned: measured from attempt start through the end of verification and stamped into every record, overriding whatever the adapter reports — no two targets can measure different intervals (round 2). Capability coherence exempts engine-owned keys.

## Budget Enforcement

- **Wall-clock: hard, for every enforcement mode.** Context deadline across run + verification; expiry aborts → `budget-overrun`.
- **Tokens/cost: hard where streamable.** Streamed usage triggers engine cancellation at the cap (malformed deltas — negative, NaN/Inf, overflowing — are rejected so they cannot bypass cancellation; round 2). `self-enforced` targets enforce their own declared caps. `post-hoc` targets are **degraded enforcement, permitted by ADR 0025 as amended 2026-07-16**: declared in every record, with overrun *rates* comparable only within one enforcement mode while cost metrics stay comparable across modes.
- **Effective budget** = story caps bounded by the bundle's `max_cost_usd_per_run`.
- **Conservative suite admission (round 2).** Admission reserves the attempt's **effective per-run maximum** — the most an admitted attempt may legitimately spend — not the smaller expectation, so the suite cannot overshoot its cap by launching. After the attempt, the charge settles to the observed cost when known (including overspend above the reservation); unknown cost keeps the full reservation, never zero.

## Suite Manifest

A partial suite must be distinguishable from a corrupt results file. Alongside `<suite-id>.jsonl` the store keeps `<suite-id>.manifest.json` — rewritten (not appended) as the suite progresses: the planned matrix (story × config × repeat), per-attempt status (`completed` with run ID / `skipped` with reason), budget accounting (cap, charged, observed), and the stop reason (`completed`, `suite-budget-exhausted`, `interrupted`). Fatal engine/persistence errors **best-effort finalize the manifest as `interrupted` before surfacing** — no exit path leaves it saying `running` (round 2) — and suite discovery unions record files and manifests, so a manifest-only interrupted suite is still listed. Item 7's reports consume it; "partial is reported as partial" becomes a recorded fact, not an inference.
