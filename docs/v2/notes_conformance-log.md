...
**What was attempted.** The architect seat was moved to `gpt-4.1`. That also makes the pairing **cross-lineage**, which is [ADR 0020](../adr/0020-review-invariant-reviewer-vs-partner.md)'s existing heterogeneity norm rather than a new policy. 0020 was under-specified on one word: it requires the reviewer to run a "distinct model", which `claude-opus-4-1` reviewing `claude-sonnet-4-6` satisfies literally while being one lab and one training lineage. DR sharpened this on 2026-08-08 to mean genuinely distinct lineages — generally different labs — with non-conforming configurations **degraded, not invalid**. Amendment pending as [backlog candidate 16](notes_adr-backlog.md).

**Retroactive consequence, recorded rather than quietly fixed:** `paired-default` has paired Opus with Sonnet since its first commit (`0649f51`, 2026-07-17), so **every row in this log above was produced by a same-lineage configuration**, and none was flagged degraded — ADR 0020 requires the degraded state to be actively surfaced, and no lineage concept exists in the code at all. Those rows remain valid measurements of what they measured; they are simply degraded-review runs that were never labelled as such. The run was stopped during the second story. Two v1 defects, both **Phase 3 work** and both filed:
