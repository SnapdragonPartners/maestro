...
**What was attempted.** The architect seat was moved to `gpt-4.1`. That also makes the pairing cross-vendor. DR directed the seat off Anthropic on 2026-08-08; the **status of that as policy is `proposed`, not accepted** — it awaits DR's approval in explicit terms. It is not a restoration either way: `paired-default` has been Anthropic-architect/Anthropic-coder since its first commit (`0649f51`, 2026-07-17), and ADR 0025 treats reviewer heterogeneity as something to *benchmark*, not a conformance requirement. The run was stopped during the second story. Two v1 defects, both **Phase 3 work** and both filed:
