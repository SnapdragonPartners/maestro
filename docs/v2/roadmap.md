...
> Maestro is an agentic factory for individuals and teams doing large-scale, production-grade software work. It converts ambiguous human intent into reviewed, evidenced, auditable changes by optimizing the Model/Prompt/Harness (MPH) system around software development best practices, reusable artifacts, task-scoped teams, governed context, and measurable outcomes.
...
- The 80/20 principle applies: approximately 80% of features can be built by this kind of automated development, but approximately 20% require a tighter interaction with an operator ("conductor" instead of "orchestrator").
...
The highest-level ask from the human. A Feature may span multiple repositories and contain many Tasks. It may also be a complete app MVP for a greenfield project.
...
MVP constraint: build the CPA's artifact contracts and interface first. The CPA is expected to be always-on as the persistent human-interface layer, but it delegates all substantial work to Task Teams and must not become a mega-agent that hides workflow logic. See D2.
...
- Each Story gets a Story branch from the Task branch head.
...
Yes. CPA/CTA are always-on agents in the sense that they are the persistent human-interface layer, holding Feature-level memory across Tasks. They are roughly the v1 PM/Architect pair with cross-Task persistence — possibly simpler. The earlier "not a mega-agent" caution was about workflow logic, not lifetime: virtually all substantial work is delegated to Task Teams, and the CPA prompt must not become the place where orchestration policy hides.
...
To keep the Task plan from duplicating the Requirements artifact, the split is:
...
- Task plan (Architect-owned): the how and the proof — repo scope, integration strategy, branch policy, story decomposition and rationale, evidence plan, risks, gates.

The Task plan cites requirements; it never restates them. If intent changes, the Requirements artifact changes first and the plan is re-derived. The artifact schemas should enforce this split, not convention.
...
This is the right human-level gate. Note that it creates deliberate back-pressure in large automated Features: the dependency graph cannot unblock downstream Tasks until upstream Tasks merge. The dashboard should make that queue visible so the operator can see when they are the bottleneck.
...
> A measurable, artifact-first agentic factory where task-scoped teams create and review production-grade changes under explicit Model/Prompt/Harness control.
