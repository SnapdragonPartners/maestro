...
- External design review completed (DR's designer contact has offered input at spike time; routed through DR per the build process).
...
- Bypass the agent, never the artifact: even the ten-second expert path emits Feature/Epic records with provenance (author = human, via intake form).
