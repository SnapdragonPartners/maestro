...
- **Cross-vendor pairing is `proposed`, not accepted — and not a restoration.** DR directed the architect seat off Anthropic on 2026-08-08, but its status as policy is unresolved and awaits explicit approval. `paired-default` has been Anthropic-architect/Anthropic-coder since its first commit, and ADR 0025 makes reviewer heterogeneity something to *benchmark*, not a mandate. If it is to bind, it needs an accepted decision rather than inheritance from a config comment.
