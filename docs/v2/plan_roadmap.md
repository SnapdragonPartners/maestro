...
- **Reviewer heterogeneity means distinct *lineage*, not distinct model** — ADR 0020's existing norm, sharpened by DR on 2026-08-08, filed as [backlog candidate 16](notes_adr-backlog.md). 0020 requires the reviewer to run a "distinct model" without defining lineage, so Opus-reviewing-Sonnet passes the letter while sharing a lab and a training lineage. Non-conforming configurations are **degraded, not invalid**. Two consequences worth carrying: `paired-default` was itself degraded from its first commit until 2026-08-08, and **ADR 0020's flagging-and-surfacing requirement has never been implemented** — nothing computes heterogeneity, so the degradation was invisible rather than accepted. DR intends this to apply everywhere possible, not only to benchmarks.
