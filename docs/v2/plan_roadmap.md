...
- **Cross-vendor pairing is a new preference, adopted by DR 2026-08-08 — not a restoration.** `paired-default` has been Anthropic-architect/Anthropic-coder since its first commit, and ADR 0025 makes reviewer heterogeneity something to *benchmark*, not a mandate. Treat it as a preference to be validated by a run, and if it is to become binding, record it as an accepted decision rather than inheriting it from a config comment.
