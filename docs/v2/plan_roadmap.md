...
- ~~**Carried from Phase 2 — discharge before v1 is removed.** Fix [#317](https://github.com/SnapdragonPartners/maestro/issues/317), then run `golden-all`, N = 1, `paired-default` against the **v1-as-patched** target and record it in the conformance log.~~ **STRUCK by DR 2026-08-16** (proposed, pending approval with the [Phase 3 plan](phase_3/plan_scope.md)). Left struck rather than deleted so the lost comparison point stays visible: this was the last opportunity to measure the v1 path, and Phase 2 had already missed its own. See the amendment above.
- Before the v1 factory path is removed, a **v2 target adapter** exists. After removal, the phase-end conformance run required by ADR 0025 — `golden-all`, N = 1, `paired-default` — completes against that adapter. Phase 3 does not exit until it does. It was described here as the **second** of two runs; with the carried checkpoint struck it is the only one, and therefore **the first measurement ever taken on the v2 path** — a baseline rather than a regression check. It still proves what it needs to: that the v2 path completes the suite on stories carrying behavioural oracles. From Phase 4 onward it is a regression test in the ordinary sense.
