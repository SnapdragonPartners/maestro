...
**Amended 2026-08-16 (Codex + DR), with the [Phase 3 plan](phase_3/plan_scope.md).** It owes **one**. DR struck the carried v1-as-patched run: #316 and #317 are defects in the v1 architect path and the v1 adapter, Phase 3 deletes both, and patching a target in order to measure it immediately before removing it produces nothing that outlives the phase. **The cost is permanent and is recorded rather than absorbed: there will never be a measured v1-to-v2 comparison point.** What survives is runner-side — #318's dirty-tree preflight and #319's model-lifecycle preflight — plus #323's architect-seat question, retargeted at the v2 loop. #316 and #317 close with v1, each leaving a requirement rather than a fix: a terminal tool must be forceable, and sampling parameters must be optional.
...
- ~~**Carried from Phase 2 — discharge before v1 is removed.** Fix [#317](https://github.com/SnapdragonPartners/maestro/issues/317), then run `golden-all`, N = 1, `paired-default` against the **v1-as-patched** target and record it in the conformance log.~~ **STRUCK by DR 2026-08-16**, Accepted with the [Phase 3 plan](phase_3/plan_scope.md). Left struck rather than deleted so the lost comparison point stays visible: this was the last opportunity to measure the v1 path, and Phase 2 had already missed its own. See the amendment above.
