...
- [Phase 2 Item 5 Design: Calls, Metrics And Audit Events](design_calls_family.md) — Mini-plan for the call family's typed queries: per-table write invariants, the open-to-completed call lifecycle, the cases the seam must reject, dependency-ordered truncation that refuses to prune pinned, open or referenced rows, and an exact decimal type for cost.
