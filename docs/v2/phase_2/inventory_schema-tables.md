...
The two families ADR 0022 names for Phase 2 that item 3 left uncreated. They
are the one place the anti-speculation rule and the ADR point in opposite
directions — both are built ahead of any consumer — and the plan's delegated
decision 1 carves them out explicitly.

| Table | Required by | Consumed by |
| --- | --- | --- |
| `configuration_records` | [0022](../../adr/0022-v2-data-plane.md) — configuration resolved into the plane | Item 7: one row per key and scope, since most-specific-wins is undefined the moment two rows share a level; the organization arm of the scope check is an **equality**, because that column references organizations directly while the other arms are tenant-bound by composite keys |
| `secrets` | [0022](../../adr/0022-v2-data-plane.md) + the [project-folder spike](../phase_0/spike_project-folder.md) | Item 7: ciphertext only, with the key derived per version outside the plane; **two partial unique indexes** rather than one constraint, because `NULL` is not equal to itself and a plain `UNIQUE` would admit unlimited duplicate shared secrets |
