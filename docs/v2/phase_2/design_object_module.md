...
**Repeating a version-specific delete has to be harmless in the ADAPTER, not in the server.** D6 rests on "repeating a version-specific delete is harmless by construction, which is what makes recovery safe to run at any time" — and the claim exists for exactly the crash that tests it: after the remote delete has landed, before the row clears. Recovery then names a version that is already gone. **Measured** against the pinned image: a repeated delete, an unknown version id on a live key, and a delete against a key that never existed all return *no error at all*. So the integration suite cannot tell whether the tolerance exists, and a store that answers `NoSuchVersion` would strand the claim forever — retried at every startup, and forbidding the existing-object shortcut for its digest the whole time. `DeleteVersion` therefore tolerates the already-gone responses exactly as `AbortUpload` does, unit-tested against a canned response, with everything else still propagating: a swallowed failure would clear a claim over storage still in the bucket, which is the leak the claim was written to prevent.
...
| **More than a budget's worth** of claimed digests sorting ahead of one reclaimable object | The reclaimable object is still reclaimed in that pass: claims are filtered out before the bound, so they can never consume it (D6c) |
| Reconciling a claim whose storage was **already deleted** before the crash | The claim clears. A repeated version-specific delete is tolerated in the adapter, since the pinned server's silence cannot be relied on (D6c) |
