...
| `recover-key` | **Allowed**, and refusing it was a defect rather than a conservative choice — corrected in round seven. The debt is recorded by a *keyless restore*, which is precisely how ADR 0022's second branch is entered; refusing recovery there made that branch reachable only for a live plane that lost its key, never for the restore the ADR describes. The error even said "run `dataplane-up`, which verifies the plane" — against a plane `up` cannot open. It is safe for the reason `up` is and by the *same mechanism* rather than a parallel one: recovery ends by calling `up` internally, so the debt reaches the identical settlement. A **compound settlement path**, not an exemption. |
| `backup`, `migrate`, `force-version` | **Refuse.** Backing up an unchecked plane is how an unchecked plane becomes an archive somebody later restores from — and the two-part restore leaves the plane *stopped and owing*, which is a state `backup` is otherwise perfectly happy to copy. `migrate` and `force-version` would apply schema changes to contents nothing has vouched for. |

So the tables agree on every refusal and differ on the two operations that can *reach* a settlement: `up`, which is the settlement, and `recover-key`, which is the only way to open a plane that cannot be opened at all. The two states share one hazard — *acting on contents nobody has vouched for* — and diverge exactly where an operation exists to end it.
...
| Operation | With an interrupted recovery |
| --- | --- |
| `recover-key` | Allowed — it *is* the resume, and removes the survivor before anything else. |
| `down` | Allowed **with an obligation of its own**: it must stop the recovery container, which `compose down` cannot reach — the container is outside the project by design. Without that, `down` returned having stopped everything *except* the postmaster that makes the state hazardous, reporting the plane stopped while the one process owning the cluster kept running. It must stop **there**: the marker and staged key are retained, because `down` is not a discard and the recovery must remain resumable. |
| `reset`, `restore` | Allowed **with an obligation**: each must remove the container and the staged key. Discarding or replacing a plane while a postmaster still holds its directory is not a discard at all — a `reset` that emptied the directory under a live server is the shared-state corruption ADR 0027 exists to prevent, reported as success. |
| `up`, `backup`, `migrate`, `force-version`, `verify` | **Refuse**, naming the marker and the two ways out. |

Note `up` sits on the **refused** side here and is permitted by both other tables. That asymmetry is the point: an unverified plane must be started, because starting it is how it gets verified; a plane with an orphaned postmaster must not be, because starting it is what causes the harm.

Enforced by the same structural completeness test as the other two tables, and reached through the same single combined guard — a verb cannot opt into part of the policy by omission, which is exactly how this marker came to gate nothing.

**The residue split, and why it is a split.** Removing the *container* destroys nothing durable — a resume rebuilds it from the marker and the staged key — so it is safe on either side of a phase boundary, and `restore` must do it *before* clearing the data root the container holds open. Removing the *marker and staged key* is destructive, so under D4's rule it may only happen after `RestoreIncompleteMarker` is written. An earlier implementation cleared both in `Restore`, before `replaceTree` wrote anything: a crash there deleted recovery state with nothing on disk recording that a destructive operation had begun — the exact window D4's marker exists to close, reopened one call above it. The ordering is enforced structurally, like D4's shutdown-arming rule and for the same reason: the defect is observable only through a crash in a few milliseconds, and both spellings read as equally careful.

**Both removals are directory-synced.** An unlink is a metadata write, and one still in the page cache when the machine loses power leaves the entry present on the next boot. The staged key is the dangerous half: resurrected beside a freshly provisioned plane, it is key material with no marker explaining it.

**An archive may not carry these markers either.** They are ordinary files in the data root, so a hand-assembled archive can contain one — and `backup` is refused against a plane carrying any of the three, so no archive this code produced ever does. Restoring one installs another plane's in-flight state: the internal `up` does not re-run the guard, so the restore reports success and hands back a plane every later verb refuses. Archives are therefore checked for all three markers **before the lock is taken and before the plane is stopped**, with the rest of the source validation. The check is distinct from the manifest inventory, which only catches a marker planted *after* the manifest was written — an archive built by other means has an honest manifest of whatever it contains.

**Two durable fields, both validated before use.** The marker records the recovery container's name and the staged key's path, and both become privileged arguments — one a filesystem path this code reads and renames, the other an argument to `docker rm --force`. Both are *derivable*, so both are compared against what this configuration would itself have written before either is used. A marker that arrives by any other route — restored inside an archive taken on another machine is the realistic case — describes a recovery this plane never started, and is refused rather than repaired.
...
15. **Refusals**: restore onto a populated root without `-force`; restore from a source missing a service directory, asserted to leave the existing plane intact; and restore from an archive carrying any of the three lifecycle markers, with the manifest **regenerated over the planted file** so the archive is structurally perfect and only its contents are wrong — planting it after the manifest is written exercises the inventory comparison instead, which a first version of this test did, and it passed against a build with the lifecycle check removed. Each refusal is asserted to leave the plane running, not merely to return an error.
...
18. **New-key recovery** (native-Linux CI): recovery over a plane with data and secrets; the data survives, secrets are gone, the new credential authenticates over the network by service name, the old one is rejected, and no listener exists during the recovery step. The rejection is asserted after first proving the two keys derive *different* passwords, or it would prove nothing; "no listener" is read from the running server's own `listen_addresses`, since absent ports and networks describe how the container is wired and neither implies the server is not binding a socket. **Plus the branch this all exists for, end to end**: restore an archive without its key, and recover the resulting locked-and-owing plane — the path D4a's first version made unreachable.
...
20. **Recovery owns its residues**: after a killed run, assert the surviving recovery container is stopped and removed before the probe, and that a staged key written without its marker is cleaned rather than adopted. **Per escape, separately**: `reset` and `restore` each get their own test, because they are not interchangeable — `reset` discards and has no pre-deletion marker, `restore` replaces and everything it deletes sits behind D4's boundary. And `down` gets both halves: the orphan is gone afterwards, *and* the marker and staged key survive so the recovery can still be resumed — asserted by resuming it.
