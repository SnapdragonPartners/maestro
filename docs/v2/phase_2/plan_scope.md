...
Status: **live** — Approved by Codex and DR, 2026-07-24, after one review round (the results-sink-versus-import question, resolved below). It flips to `archive` at phase close. (Phase 1's plan was merged still `draft` and needed a follow-up flip PR; this document is flipped before its own merge to avoid repeating that.)
...
- **The cross-store commit-order invariant** — object, then the attachment row, then the referencing artifact and its retention pins in one transaction, with acceptance verifying that every referenced object exists and every pin matches its attachment's digest (ADR 0022, amended 2026-07-29) — enforced at the seam and tested under failure injection, not left as an implementation convention.
...
| 6 | `objects` | The object module and its MinIO-backed S3-compatible adapter: put/get by digest, exists, pin/unpin, delete-unpinned. The **cross-store commit-order invariant** enforced at the seam — object, attachment row, then artifact and pins in one transaction, with acceptance as the verifying step (ADR 0022, amended 2026-07-29) — with failure-injection tests at each step proving no row ever references a missing or prunable blob, and digest verification on read so a retention bug fails loudly rather than silently weakening a proof. | M |
...
Codex answered all five on 2026-07-24; DR confirmation rides on this document's approval. Every question resolved as proposed.

1. **Should the artifact-envelopes ADR be authored inside the phase (item 1) or Accepted before Phase 2 opens?** **Resolved: inside, as item 1.** The backlog's rule is that a blocking entry is Accepted "before its blocking phase starts *implementation*", which item 1 satisfies exactly — it precedes item 3's DDL. Authoring it as a pre-phase step would add a review round and a branch outside any phase for no additional safety.
2. **Reserved families: name-only, or empty DDL now?** **Resolved: name-only** (delegated decision 1), with configuration and secrets built anyway because ADR 0022 names them for Phase 2. Worth confirming, because it means the Phase 2 schema will look *smaller* than ADR 0022's sixteen-family list, and a reviewer expecting the full list should see that as intended rather than as an omission.
3. **`internal/dataplane/` as a new package, with `pkg/persistence` deleted rather than edited in Phase 3?** **Resolved: yes** (delegated decision 2). This is the mechanism that keeps the v1 target alive through the phase, but it does mean the port inventory's "rewrite in Phase 2" disposition completes in Phase 3.
4. **Is Phase 2 the right home for the secrets vault and cold backup?** **Resolved: yes, per ADR 0022**, which says so explicitly. Flagged because both are built ahead of any consumer, which is the one place this plan's own anti-speculation rule and the ADR point in opposite directions. If a reviewer prefers to retime them to Phase 3, that is an ADR 0022 amendment, not a plan change.
5. **Should the plane be a results *sink* the runner writes to, or an import *destination*?** The one contested point in review: Codex's round-1 edit made the plane a runner-held sink, this plan restored import, and **Codex resolved it to import destination** — explicitly declining both an ADR 0025 amendment and the subprocess-triggered live-import variant *for now*. Three things were agreed throughout and are in the text either way: golden records land in the **main** database and never a benchmark-private one; the write path is append-only and idempotent by stable run/attempt identity, rejecting conflicting payloads rather than overwriting; and a plane outage never invalidates a completed run. The disagreement was only about *when* the write happens, never about those outcomes.

   **Resolved: import destination**, which is what ADR 0025 says ("zero dependency on the Phase 2 data plane… Phase 2's vertical slice does that import") and ADR 0022 assumes ("imports later"). Three concrete costs argue against a runner-held sink, beyond the ADR text: `benchmark/` is a separate Go module whose only dependencies today are `BurntSushi/toml` and `modernc.org/sqlite`, and it cannot import `internal/dataplane` across that boundary — so a sink means a Postgres driver plus a duplicated schema in the module whose black-box isolation is *structurally* enforced (the reason it was made a separate module in Phase 1); ADR 0022's access discipline routes all plane access through the Orchestrator's persistence seam, which the runner is not; and it adds a failure mode to the measuring instrument during the one phase whose top risk is breaking it.

   **The liveness option is parked, not adopted.** Per-attempt writes are reachable without breaking any of the above — the runner would invoke the importer as a **subprocess**, an external surface, exactly how it already drives targets. Codex declined it for Phase 2, so ADR 0025 is **not** amended and no live-import path is built here. Parked in the [parking lot](../notes_parking-lot.md) so the option survives this document's archival; picking it up later still requires the ADR 0025 amendment, since it promotes the plane from import destination to results sink.
