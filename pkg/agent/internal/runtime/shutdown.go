...
func (d *ShutdownableDriver) CanResume() bool {
	baseStateMachine, ok := utils.SafeAssert[*core.BaseStateMachine](d.StateMachine)
	if !ok {
		return false
	}
...
func (d *ShutdownableDriver) Resume(_ context.Context) error {
	...
	baseStateMachine2, ok2 := utils.SafeAssert[*core.BaseStateMachine](d.StateMachine)
	if !ok2 {
		return fmt.Errorf("state machine is not a core.BaseStateMachine")
	}
