// Package resilience provides circuit breaker, retry, and timeout patterns for reliable agent communication.
package resilience
...
// CircuitBreakerClient wraps an llm.LLMClient with circuit breaker pattern.
...
func NewCircuitBreakerClient(client llm.LLMClient, config CircuitBreakerConfig) *CircuitBreakerClient {
...
// Complete implements llm.LLMClient with circuit breaker logic.
func (cb *CircuitBreakerClient) Complete(ctx context.Context, req llm.CompletionRequest) (llm.CompletionResponse, error) {
...
// Stream implements llm.LLMClient with circuit breaker logic.
func (cb *CircuitBreakerClient) Stream(ctx context.Context, req llm.CompletionRequest) (<-chan llm.StreamChunk, error) {
	// Check if we can proceed.
	if err := cb.allowRequest(); err != nil {
		return nil, err
	}
...
func NewResilientClient(baseClient llm.LLMClient) llm.LLMClient {
...
func NewResilientClientWithLogger(baseClient llm.LLMClient, logger *logx.Logger) llm.LLMClient {
	...
	return NewRetryableClientWithLogger(cbClient, retryConfig, logger)
}
