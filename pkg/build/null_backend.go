...
func (n *NullBackend) Build(ctx context.Context, root string, stream io.Writer) error {
...
func (n *NullBackend) Test(ctx context.Context, root string, stream io.Writer) error {
	if !n.isEmptyRepo(root) {
		return fmt.Errorf("null backend should only be used for empty repositories")
	}
...
func (n *NullBackend) Lint(ctx context.Context, root string, stream io.Writer) error {
	if !n.isEmptyRepo(root) {
		return fmt.Errorf("null backend should only be used for empty repositories")
	}
...
func (n *NullBackend) Run(ctx context.Context, root string, args []string, stream io.Writer) error {
	if !n.isEmptyRepo(root) {
		return fmt.Errorf("null backend should only be used for empty repositories")
	}
