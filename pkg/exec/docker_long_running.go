...
func (d *LongRunningDockerExec) StartContainer(ctx context.Context, storyID string, opts *Opts) (string, error) {
	...
	args = append(args, "--volume", "/var/run/docker.sock:/var/run/docker.sock", "--tmpfs", fmt.Sprintf("/tmp:exec,nodev,nosuid,size=%s", defaultTmpfsSize), "--tmpfs", "/home:exec,nodev,nosuid,size=100m", "--tmpfs", "/.cache:exec,nodev,nosuid,size=100m")
