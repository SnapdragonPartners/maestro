...
func TestRenderBootstrapSpec_Generic(t *testing.T) {
	...
	assertContains(t, result, "Generated by Maestro Bootstrap System")
...
func TestAllPacksBindToTemplate(t *testing.T) {
	...
	for _, packName := range packNames {
		t.Run(packName, func(t *testing.T) {
			...
			// Verify pack display name is present in platform info (case-insensitive)
			// Some platforms like "generic" may not have capitalized display names in output
			pack, _, _ := packs.Get(packName)
			if pack != nil {
				if !strings.Contains(strings.ToLower(result), strings.ToLower(pack.DisplayName)) {
					t.Errorf("Expected output to contain %q (case-insensitive)", pack.DisplayName)
				}
			}
		})
	}
