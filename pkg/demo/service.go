...
func (s *Service) startContainerOnly(_ context.Context) error {
	// For MVP, we'll require a compose file
	// This could be extended to run a container directly using config.Build.RunCmd
	return fmt.Errorf("demo requires a compose file at %s", ComposeFilePath(s.workspacePath))
}
