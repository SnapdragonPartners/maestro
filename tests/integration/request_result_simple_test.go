...
func TestPlanCodeHappyPath(t *testing.T) {
	...
	// Initialize coder.
	if initErr := coderAgent.Initialize(ctx); initErr != nil {
	...
	err = coderAgent.ProcessApprovalResult(context.Background(), "APPROVED", "plan")
	if err != nil {
	...
	for i := 0; i < maxSteps; i++ {
		...
		done, stepErr := coderAgent.Step(ctx)
		if stepErr != nil {
			// Ignore "no more responses" errors but stop stepping.
			if stepErr.Error() == "mock client: no more responses" {
				break
			}
			t.Logf("Step %d error: %v", i, stepErr)
		}
	...
	err = coderAgent.ProcessApprovalResult(context.Background(), "APPROVED", "code")
	if err != nil {
