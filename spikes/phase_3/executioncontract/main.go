...
func (r *runner) report() int {
	...
	fmt.Println("  model calls, token accounting and per-model-call provenance bindings —")
	fmt.Println("  the stub backend makes no model calls, and the ordinary path emits no")
	fmt.Println("  provenance event rather than a hollow one. The delivery scenarios do emit")
	fmt.Println("  a synthetic `usage` envelope as a TRANSPORT FIXTURE; the claims count")
	fmt.Println("  envelopes and never read them, so nothing concludes accounting from it.")
