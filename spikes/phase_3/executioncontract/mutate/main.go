...
func mutations() []mutation {
	return []mutation{
		...
		{
			name:  "reconciler-sweeps-declared-waits",
			file:  "host/boundary.go",
			old:   "\t\tif att.State != StateOpen {\n\t\t\tcontinue\n\t\t}",
			new:   "\t\tif att.State == StateSettled {\n\t\t\tcontinue\n\t\t}",
			claim: "gate/headless-blocks-immediately",
			want:  "blocked without a requirement reference",
			why:   "THE DEFECT THIS SUITE ACTUALLY FOUND: reconciliation settling an attempt that is healthily waiting on an operator, destroying the requirement the blocked result must reference",
		},
		{
			name:  "reconciler-settles-nothing",
			file:  "host/boundary.go",
			old:   "\t\tif att.State != StateOpen {\n\t\t\tcontinue\n\t\t}",
			new:   "\t\tif att.State != StateOpen || true {\n\t\t\tcontinue\n\t\t}",
		...
		{
			name:  "at-most-once-mints-a-second-attempt",
			file:  "host/boundary.go",
			old:   "\tif existing, ok := r.byCorrelation[key]; ok {",
			new:   "\tif existing, ok := r.byCorrelation[key]; ok && false {",
			claim: "boundary/at-most-once",
			want:  "the effect committed 2 times",
			why:   "a transport retry treated as a new action is how an adapted runtime duplicates a forge push",
		},
		{
			name:  "terminal-recorded-on-unconfirmed-fence",
			file:  "host/runtime.go",
			old:   "\tif out.Result.Status == contract.StatusCancelled && out.FenceReceipt == FenceUnconfirmed {",
			new:   "\tif false {",
